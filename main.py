...
st.info(
    "영화를 선택했을때 날짜별 영화의 일관객 변화를 볼 수 있다. "
    "또 시간이 지남에 따라 관객 수가 증가하는지 감소하는지 알 수 있게 될 수 있다."
    "각 영화를 선택했을때 그래프가 다 다르다."
)
...
st.info(
    "모든 기간동안의 일관객 합계가 큰 TOP 5 영화의 날짜별 관객을 서로 비교해 "
    "언제 관객이 증가하고 감소하는지 그 시점을 알 수 있고 흥행 시점을 알 수 있다."
    "스파이더맨이 개봉됐을떄 엄청 흥행했지만 오디세이가 개봉하자 떨어졌다."
)
...
st.info(
    "날짜별로 박스오피스에서 10위권 영화의 일관객 합계를 비교해  "
    "영화관 관객 규모가 증가, 감소 시기를 알게 되어 "
    "가장 많았던 날을 알 수 있다."
    "12월 25일에 가장 많았다."
)
...
st.info(
    "일정 시간 동안 일관객 합계가 큰 영화들을 비교해 "
    "무슨 영화가 가장 많은 관객을 모았는지 "
    "그리고 10위권에 등장한 날수를 알 수 있다."
    "왕과 사는 남자가 제일 많다."
)
...
st.info(
    "월 과 요일에 따라 일관객 합계를 색의 진하기로 비교했다 "
    "어떤 월과 요일에 박스오피스 10위권 관객이 많았는지 알 수 있다."
    "8월 토요일이 가장 많다."
)
